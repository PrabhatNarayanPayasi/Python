# f = open('Day50(file methods)\h.txt' ,'r');
# print(f.read())
# f.seek(4);
# print(f.read(6))
# seek function btata hai ki kaha tak file me words ko chhod den hai aur kaha se read karna hai
# read method ekk argument leti hai jo ye btati hai ki kitne chaacter red karne hai.
 
# f =  open('Day50(file methods)\\h.txt' , 'w');
# f.write("Kya haal chal hai bhai")
# f.truncate(2);
# ISKA MATLB HAI KI M CHAHT HU KI ab jo file open hui hai uska size 2byte ka ho to mine truncate(2) likha hai
# f = open('Day50(file methods)\\h.txt' ,'r');
# print(f.read())


# tell function current position btata hai
...
with open('Day50(file methods)\\h.txt', 'r') as f:
  # Read the first 10 bytes
  data = f.read(9)
  # Save the current position
  current_position = f.tell()
